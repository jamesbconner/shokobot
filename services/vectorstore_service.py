...
def _embeddings() -> OpenAIEmbeddings:
    """Create and cache OpenAI embeddings instance.
    ...
    """
    model = _cfg.get("openai.embedding_model")
    if not model:
        raise ValueError("openai.embedding_model not configured")
    
    timeout = int(_cfg.get("openai.request_timeout_s", 60))
...
def get_chroma_vectorstore() -> Chroma:
    ...
    collection_name = _cfg.get("chroma.collection_name")
    
    if not persist_dir or not collection_name:
        raise ValueError("Chroma configuration incomplete: missing persist_directory or collection_name")
...
def delete_by_anime_ids(anime_ids: Sequence[str]) -> None:
...
def upsert_documents(docs: list[Document]) -> list[str]:
